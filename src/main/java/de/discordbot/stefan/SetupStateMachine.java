...
public class SetupStateMachine extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if (event.getAuthor().getIdLong() != this.userId) return;
        if (channel.getIdLong() != this.channelId) return;
        ...
        switch (this.state) {
            case 0:
                ...
                if (guestRole == null) {
                    channel.sendMessage("Can't find role. Please send a message with the @ of the role.").queue();
                } else {
                    this.guestRoleId = guestRole.getIdLong();
                    channel.sendMessage("Please enter the channel in which the guests start with the # notation.").queue();
                    this.state = 1;
                }
                break;
            case 1:
                ...
                if (guestChannel == null) {
                    channel.sendMessage("Can't find that channel. Please send a message with the # of the text channel.").queue();
                } else {
                    this.guestChannelId = guestChannel.getIdLong();
                    channel.sendMessage("Please enter the channel to which I should send messages to admins.").queue();
                    this.state = 2;
                }
                break;
            case 2:
    ...
    private void onSetupComplete(Guild guild) {
        ...
        try {
            if (set.next()) {
                LiteSQL.onUpdate(String.format("UPDATE guildinfo SET guestroleid = %d, guestchannelid = %d, adminchannelid = %d WHERE guildid = %d", this.guestRoleId, this.guestChannelId, this.adminChannelId, guild.getIdLong()));
            } else {
                LiteSQL.onUpdate(String.format("INSERT INTO guildinfo(guildid, guestroleid, guestchannelid, adminchannelid) VALUES (%d, %d, %d, %d)", guild.getIdLong(), this.guestRoleId, this.guestChannelId, this.adminChannelId));
            }
